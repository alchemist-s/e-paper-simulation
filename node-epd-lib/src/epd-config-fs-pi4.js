...
class EPDConfigFSPi4 {
  ...
  async init() {
    try {
      console.log("Initializing e-paper hardware (Pi 4 File System GPIO)...");
  ...
  async setupGPIO() {
    try {
      ...
      console.log("Cleaning up any existing GPIO exports...");

      // First, try to unexport any existing pins
      for (const pin of [...outputPins, ...inputPins]) {
        try {
          ...
          // Ignore errors - pin might not be exported
        }
      }
